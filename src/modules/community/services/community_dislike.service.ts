...
import { CreateCommunityDisLikeDto } from "../dto/create-community_dislike.dto";
import { FilterCommunityDisLikeDto } from "../dto/filter-community_dislike.dto";
import { UpdateCommunityDisLikeDto } from "../dto/update-community_dislike.dto";
import { CommunityDisLike, CommunityDisLikeDocument } from "../schemas/community_dislike.schema";

@Injectable()
export class CommunityDisLikeService {
  constructor(
    @InjectModel(CommunityDisLike.name)
    private communityLikeModel: Model<CommunityDisLikeDocument>
  ) {}
   ...
   * @returns
   */
  async create(createUser: CreateCommunityDisLikeDto): Promise<CommunityDisLike> {
   ...
   * @returns
   */
  async getCondition(filter: FilterCommunityDisLikeDto) {
    let condition: any = {};
    if (filter.user_id) {
   ...
   * @returns
   */
  async removeOne(dataToSearch: any): Promise<CommunityDisLike> {
   ...
   * @returns
   */
  async findById(id: string, projection: any): Promise<CommunityDisLike> {
    if (!id) {
      return null;
    }
   ...
   * @returns
   */
  async findAll(): Promise<CommunityDisLike[]> {
   ...
   * @returns
   */
  async findOne(dataToSearch: any, isWithUser: boolean = false): Promise<CommunityDisLike> {
    if (isWithUser) {
   ...
   * @returns
   */
  async update(dataUpdate: UpdateCommunityDisLikeDto) {
    try {
   ...
   * @returns
   */
  async updateWithoutCreate(dataUpdate: UpdateCommunityDisLikeDto) {
    try {
      if (!dataUpdate.user_id && !dataUpdate.community_id) {
        return null;
      }
   ...
   */
  async filter(
    filter: FilterCommunityDisLikeDto,
    sortBy: any,
    page: number,
    limit: number,
    projection: any = {}

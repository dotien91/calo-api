...
export class LivestreamHelper {
  ...
  async createNewLivestream(createLivestreamData: CreateLivestreamDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      const userOptionData = await this.userOptionService.findById(userObject?._id.toString(), {});
  ...
  async updateLivestream(dataUpdate: UpdateLivestreamDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      if (dataUpdate.livestream_status === "live") {
        ...
        if (firstData?.livestream_status !== "live") {
          setTimeout(async () => {
            ...
            //Update dataPost
            const dataTitle = userObject?.display_name + " đang livestream!";
      ...
      const eventObject = await this.eventService.findOne({ livestream_id: dataCreate?._id?.toString() });
      if (eventObject) {
    ...
    try {
      ...
      if (objectId) {
        ...
        if (dataReturn) {
          ...
          const eventObject: any = await this.eventService.findOne({ livestream_id: dataReturn?._id?.toString() });
          if (eventObject) {
  ...
  async handleSendNotificationToAllNow(
    ...
    try {
      ...
      const userIdArray = [];
      const channelId = dataLivestream?.channel_id?.toString();
      const emailArray = [];
      if (channelId) {
  ...
  async handleSendNotificationToAllCreate(
    ...
    try {
      ...
      const userIdArray = [];
      const channelId = channelObject?._id?.toString();
      const emailArray = [];
      if (channelId) {
        for (let itemPage: number = 1; itemPage <= 10; itemPage++) {
          const allUser = await this.channelPermissionService.filter({ channel_id: channelId }, {}, itemPage, 1000);

...
export class PostHelper {
  ...
  async handleUpdateLength() {
    let dataAnonymousService = await this.postAnonymousService.filter({}, {}, 1, 20000);
    for (let dataItem of dataAnonymousService) {
  ...
  async createNewPost(createPostData: CreatePostDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      let dataPermission = await this.userPermissionService.isHavePermission(userId, "post/create");
      ...
      let dataCreate = await this.postService.create(createPostData);
      return res
  ...
  async importPrompt(dataImport: CreateImportPromptDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      //DataImport
      let dataImportText = dataImport?.text;
      if (!dataImportText) {
        throw new BadRequestException("Not have data!");
      } else {
        //Update
        let dataArray = dataImportText?.split("\n");
        // console.log(dataArray);
        for (let dataText of dataArray) {
          if (dataText && dataText?.trim()) {
            //Process Text
            let dataProcess = await this.processText(dataText, dataImport?.category_id, userId);
  ...
  async createNewPostPrompt(createPostData: CreatePostPromptDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      let dataCreate = await this.postPromptService.create(createPostData);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
        .status(HttpStatus.OK)
        .json(dataCreate);
  ...
  async createNewUserPrompt(createPostData: CreateUserPromptDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      let dataCreate = await this.postAnonymousService.create(createPostData);
      let dataReturn = await this.postAnonymousService.findById(dataCreate?._id?.toString());

      let categoryId = dataReturn?.prompt_id?.post_category?._id;
      let postId = dataReturn?.prompt_id?._id;
      //Update for
  ...
  async createNewCrawlPost(createPostData: CreatePostCrawlDto, res: Response, req: ExpressRequestDto) {
    try {
      let dataCreate = await this.postCrawlService.create(createPostData);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
        .status(HttpStatus.OK)
        .json(dataCreate);
  ...
  async updateCrawlData() {
    try {
      ...
      let dataTotal = await this.postCrawlService.filter(dataCrawlFilter, {}, 1, 10000);
      if (dataTotal) {
        for (let itemTotal of dataTotal) {
          console.log(itemTotal, "itemTotal");
          try {
            let dataJson = itemTotal.data?.toString();
            let dataObject = JSON.parse(dataJson);

            let dataSearch = {
              post_slug: dataObject?.data?.slug,
            };
            let dataReturn = await this.postService.findOne(dataSearch);
            ...
            if (dataObject?.data?.categories) {
              let categorySlug = "";
              let listCategory = ["scripts", "addons", "maps", "skins", "texture-packs", "seeds"];
              for (let dataCategory of dataObject?.data?.categories) {
            ...
            let max = 10000000;
            let min = 50000;
            let dataRandom = Math.floor(Math.random() * (max - min + 1) + min);
            dataToCreate = {
              ...dataToCreate,
              ...{
                post_language: "en",
            ...
            if (dataReturn) {
              ...
              let dataUpdate = await this.postService.update(dataToCreate);
              console.log(dataUpdate, "dataUpdate");
            } else {
              //Create
              let dataCreate = await this.postService.create(dataToCreate);
  ...
  async updateCategory() {
    try {
      ...
      let dataTotal = await this.postService.filter(dataCrawlFilter, {}, 1, 10000);
      if (dataTotal) {
        for (let itemTotal of dataTotal) {
          //console.log(itemTotal, 'itemTotal');
  ...
  async updateImage() {
    try {
      ...
      let dataTotal: any = await this.postService.filter(dataCrawlFilter, {}, 1, 10000);
      if (dataTotal) {
        let count = 0;
        for (let itemTotal of dataTotal) {
          //console.log(itemTotal, 'itemTotal');
  ...
  async updateCrawlFirst() {
    try {
      ...
      if (dataTotal) {
        for (let itemTotal of dataTotal) {
          let postSlug = itemTotal?.post_slug;
  ...
  async getPostListByAdmin(query: ListPostDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      if (query?.post_category) {
        //Check category
        let totalCategoryFilter = {
          category_parent: query?.post_category,
        };
        let dataCategory = await this.postCategoryService.filter(totalCategoryFilter, {}, 1, 100);
        let dataToFilterCategory = [query?.post_category];
      ...
      let dataReturn = await this.postService.filter(dataToFilter, orderByOBject, page, limit);
      let dataCount = await this.postService.count(dataToFilter);
      return res
  ...
  async getPostListPrompt(query: ListPostDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      delete dataToFilter.limit;
      delete dataToFilter.order_by;
      let dataReturn = await this.postPromptService.filter(dataToFilter, orderByOBject, page, limit);
      let dataCount = await this.postPromptService.count(dataToFilter);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count", "X-Total-Count": dataCount })
        .status(HttpStatus.OK)
        .json(dataReturn);
  ...
  async getPostUserList(query: ListPostDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      delete dataToFilter.limit;
      delete dataToFilter.order_by;
      let dataReturn = await this.postAnonymousService.filter(dataToFilter, orderByOBject, page, limit);
      let dataCount = await this.postAnonymousService.count(dataToFilter);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count", "X-Total-Count": dataCount })
        .status(HttpStatus.OK)
        .json(dataReturn);
  ...
  async handleGetDetailPost(id: string, res: Response, req: ExpressRequestDto) {
    try {
      ...
      let dataReturn = await this.postService.findOne(dataToFilter);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
        .status(HttpStatus.OK)
        .json(dataReturn);
  ...
  async handleGetDetailPostPrompt(id: string, res: Response, req: ExpressRequestDto) {
    try {
      ...
      let dataReturn = await this.postPromptService.findOne(dataToFilter);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
        .status(HttpStatus.OK)
        .json(dataReturn);
  ...
  async handleUpdatePostByAdmin(dataUpdate: UpdatePostDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      let dataPermission = await this.userPermissionService.isHavePermission(userId, "post/update");
      if (process.env.BRANCH_NAME !== "mojo" && process.env.BRANCH_NAME !== "revu" && !dataPermission) {
        throw new BadRequestException("You haven't permission for this Action!");
      }
      ...
      let dataReturn = await this.postService.update(dataUpdate);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
        .status(HttpStatus.OK)
        .json(dataReturn);
  ...
  async handleUpdatePostPromptByAdmin(dataUpdate: UpdatePostPromptDto, res: Response, req: ExpressRequestDto) {
    try {
      ...
      if (dataUpdate.placeholder) {
        ...
        let objectPrepare = JSON.parse(dataUpdate.placeholder);
        for (let dataItem of objectPrepare) {
          if (dataItem) {
      ...
      let dataReturn = await this.postPromptService.update(dataUpdate);
      return res
        .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
        .status(HttpStatus.OK)
        .json(dataReturn);
  ...
  async handleDeletePost(id: string, res: Response, req: ExpressRequestDto) {
    try {
      ...
      if (await this.userPermissionService.isHavePermission(userId, "post/delete")) {
        //Check Permission
        let dataReturn = await this.postService.remove(id);
        return res
          .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
          .status(HttpStatus.OK)
          .json(dataReturn);
  ...
  async handleDeletePostPrompt(id: string, res: Response, req: ExpressRequestDto) {
    try {
      ...
      if (await this.userPermissionService.isHavePermission(userId, "post/delete")) {
        //Check Permission
        let dataReturn = await this.postPromptService.remove(id);
        return res
          .set({ "Access-Control-Expose-Headers": "X-Authorization, X-Total-Count" })
          .status(HttpStatus.OK)
          .json(dataReturn);

...
class HookExpress {
  ...
  add_action(tag: string, callback: Promise<any> | Function, priority?: number) {
    ...
    if (this.callbackHash.indexOf(_callbackHash) > -1) {
      console.log(Bright + BgMagenta + ' May be add_action runing wrong: callback function exist in this hook: %s ' + Reset, tag);
      return;
    } else {
  ...
  */
  add_filter(tag: string, callback: (...args: any) => any, priority?: number) {
    ...
    if (this.callbackHash.indexOf(_callbackHash) > -1) {
      console.log(Bright + BgBlue + ' May be add_filter runing wrong: callback function exist in this hook: %s ' + Reset, tag);
      return;
    } else {
      this.callbackHash.push(_callbackHash);
    }
